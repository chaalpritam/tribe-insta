...
/// `TribeService` maps `HubUser → User` for the view layer.
public struct HubUser: Decodable, Identifiable, Hashable {
    ...
    public let profile: HubUserProfile?
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.profile = try c.decodeIfPresent(HubUserProfile.self, forKey: .profile)
    }
}

public struct HubUserProfile: Decodable, Hashable {
...
public struct HubUserListResponse: Decodable {
    public let users: [HubUser]
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let u = try? c.decode([HubUser].self, forKey: .users) {
            self.users = u
        } else if let u = try? c.decode([HubUser].self, forKey: .followers) {
            self.users = u
        } else if let u = try? c.decode([HubUser].self, forKey: .following) {
            self.users = u
        } else {
            self.users = []
        }
