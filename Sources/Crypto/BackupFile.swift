...
struct BackupFile: Codable, Equatable {
    ...
    private static func pbkdf2(
        ...
        guard status == kCCSuccess else { throw BackupError.decryptionFailed }
        return derived
    }
}
...
extension BackupFile {
    ...
    func apply() throws -> (tid: String, appKey: AppKey, walletAddress: String?) {
        ...
        // DM key is optional and the iOS insta surface doesn't use it
        // yet (DMs aren't an Instagram-shaped concept). Still save the
        // seed bytes to Keychain so a later port of NaClBox/DMKey can
        // pick them up without forcing a re-import.
        ...
        // browserWallet is opaque on iOS — round-tripped so a later
