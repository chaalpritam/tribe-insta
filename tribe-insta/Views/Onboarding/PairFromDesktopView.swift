...
/// AppState's phase recompute then routes into the main shell.
struct PairFromDesktopView: View {
    ...
    private func handle(_ raw: String) {
        ...
        do {
            ...
            // Set the hub first so the main shell starts fetching against
            // the right hub on phase flip.
            state.hubBaseURL = url
            try state.adopt(tid: payload.tid, appKey: key)
        } catch {
